...
public class UserServiceImp implements  UserService{
    ...
    public List<User> getTodayUsers() {
        ...
        return userRepository.findBySignUpDate(today);
    }

    @Override
    public User updateUser(User userDetails) {
                User existingUser = userRepository.findById(userDetails.getId())
                .orElseThrow(() -> new IllegalStateException("User not found with id " + userDetails.getId()));

        // Update fields
        existingUser.setFullName(userDetails.getFullName());
        existingUser.setEmail(userDetails.getEmail());
        existingUser.setContactNumber(userDetails.getContactNumber());
        existingUser.setSignUpDate(userDetails.getSignUpDate());
        existingUser.setBlocked(userDetails.getBlocked());
